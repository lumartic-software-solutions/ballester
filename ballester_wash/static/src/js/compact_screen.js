...
*/
odoo.define('ballester_wash.comapct_screen', function (require) {
"use strict";
...
var CompactScreen = Widget.extend({
...
// Create   wash
	...
	init: function(parent, context) {
        ...
        var operation_data = [];
        var self = this;
        if (context.tag == 'ballester_compact_contine_screen_act') {
            self._rpc({
...
ProductOnChangeEvent: function (event)
    {	
        ...
        .then(function (result) {
		console.log("OOOOOOOOresultOOOOOOOO",result)
		if(result.lot_list != [])
		{
...
transfertocompact_store: function(event){
	...
	//var number_of_barcode = $("#barcode_number").val();
        var number_of_barcode = self.barcode
	var material_qty = $("#qty").val();
        console.log(">>>>>>>>barcodebarcode>>>>>>>>>>>",self.barcode)
	var ctx = {};
...
core.action_registry.add('ballester_compact_contine_screen_act', CompactScreen);
