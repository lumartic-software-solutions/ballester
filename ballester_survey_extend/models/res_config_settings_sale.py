...
class ResConfigSettings(models.TransientModel):
    ...
    new_compliance_id = fields.Many2one('survey.survey', 'New Compliance')
    recycled_compliance_id = fields.Many2one('survey.survey', 'Recycled Compliance')
    ...
    def get_values(self):
        ...
        new_compliance_id  = literal_eval(cp_obj.get_param('new_compliance_id', default='False'))
        recycled_compliance_id  = literal_eval(cp_obj.get_param('recycled_compliance_id', default='False'))
        res.update(
            sddr_compliance_id=sddr_compliance_id,
            waste_compliance_id=waste_compliance_id ,
  	    new_compliance_id=new_compliance_id,
	    recycled_compliance_id=recycled_compliance_id,
        )
        return res
    ...
    def set_values(self):
        ...
        cp_obj.sudo().set_param('new_compliance_id', self.new_compliance_id.id )
        cp_obj.sudo().set_param('recycled_compliance_id', self.recycled_compliance_id.id)
