...
class Purchaseorder(models.Model):
    ...
    purchase_type = fields.Selection([('minor', 'Minor Purchases'), ('complex', 'Complex Purchases'), ], 'Type of Purchase')
    partner_shipping_id = fields.Many2one('res.partner', string='Delivery Address', readonly=True, required=True, compute='_delivery_address', states={
    ...
    def _update_reserved_quantity(self, need, available_quantity, location_id, lot_id=None, package_id=None, owner_id=None, strict=True):
        ...
        try:
            if not float_is_zero(taken_quantity, precision_rounding=self.product_id.uom_id.rounding):
                ...
                quants = self.env['stock.quant'].with_context(lot_ids = self.sale_line_id.lot_ids)._update_reserved_quantity(
            ...
            if move.location_id.should_bypass_reservation()\
                ...
                if move.product_id.tracking == 'serial' and (move.picking_type_id.use_create_lots or move.picking_type_id.use_existing_lots):
                    ...
                    for i ,lot in zip(range(0, int(missing_reserved_quantity)), move.lot_ids):
                        self.env['stock.move.line'].create(move.with_context(lot=lot)._prepare_move_line_vals(quantity=1))
                else:
                ...
                print ("ddddddddddddddddddddddddddddddddddd",move.location_id)
